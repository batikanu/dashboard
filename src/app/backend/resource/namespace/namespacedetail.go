...
func toNamespaceDetail(namespace api.Namespace, events common.EventList, resourceQuotaList *resourcequota.ResourceQuotaList) NamespaceDetail {
...
func getResourceQuotas(client k8sClient.ResourceQuotasNamespacer, namespace api.Namespace) (*resourcequota.ResourceQuotaList, error) {
	list, err := client.ResourceQuotas(namespace.Name).List(listEverything)

	result := &resourcequota.ResourceQuotaList{
		Items:    make([]resourcequota.ResourceQuota, 0),
