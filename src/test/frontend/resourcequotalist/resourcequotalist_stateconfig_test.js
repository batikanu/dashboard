...
import {resolveResourceQuotaList} from 'resourcequotalist/resourcequotalist_stateconfig';

describe('StateConfig for resource quota controller list', () => {
  ...
  it('should resolve resource quota list with namespace', angular.mock.inject(($q) => {
    ...
    let actual = resolveResourceQuotaList(resource, {}, kdPaginationService);

    expect(resource.get).toHaveBeenCalledWith(kdPaginationService.getDefaultResourceQuery(''));
    expect(actual).toBe(promise);
  }));

  it('should resolve resource quota list with no namespace', angular.mock.inject(($q) => {
    let promise = $q.defer().promise;
    ...
    let actual = resolveResourceQuotaList(resource, {namespace: 'foo'}, kdPaginationService);
